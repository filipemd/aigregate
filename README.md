...
To run it, obtain a free Gemini API key, set it as the `GEMINI_API_KEY` environment variable, and run the `scripts/create_news_summary.py` (after installing the PIP packages from `requirements.txt`) script, specifying a Markdown file as the output parameter.
